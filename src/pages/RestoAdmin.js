...
export default function RestoAdmin() {
  ...
  // small UI-only state for mobile view modal
  ...
  // helper to support mobile button calls (won't change existing handlers)
  const openEditModal = (key, item) => {
    editCustomerHandler(key, item);
  };

  // tiny UI-only view modal helpers (non-destructive)
  const openViewModal = (cust) => setViewCustomer(cust);
  const closeViewModal = () => setViewCustomer(null);
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 font-poppins flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto py-8 px-4 sm:px-6">
        ...
        <div className="md:hidden space-y-4">
          {currentCustomers.length > 0 ? (
            currentCustomers.map(([key, item]) => {
              const cust = { ...item, id: key }; // normalize to use id in mobile card
              return (
                <div
                  key={key}
                  ...
                  <div className="flex justify-between items-start gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-800">
                        {cust.checkInTime || cust.checkInDate || "—"}
                      </div>
                    </div>
                  </div>
        ...
        {/* View Modal (mobile) - simple UI-only modal */}
        {viewCustomer && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
            ...
            />
            <div className="relative z-70 bg-white rounded-2xl p-5 w-full max-w-md shadow-lg">
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-800">
                ...
                <div>
                  ...
                  <div>{viewCustomer.mobile}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">DOB</div>
                  <div>{viewCustomer.dob || "—"}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Gender</div>
                  <div>{viewCustomer.gender || "—"}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">Check-in</div>
                  <div>{viewCustomer.checkInDate || viewCustomer.checkInTime || "—"}</div>
                </div>
              </div>
        ...
        {/* ================= TABLE (md and up) ================= */}
        {activeSection === "list" && (
          <div className="hidden md:block bg-white rounded-2xl shadow-lg p-5 sm:p-8 border border-gray-100 overflow-x-auto">
            <h2 className="text-lg sm:text-xl font-semibold text-blue-700 mb-6 flex items-center gap-2">
              <Users /> Customer List
            </h2>
