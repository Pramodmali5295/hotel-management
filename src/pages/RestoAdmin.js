...
export default function RestoAdmin() {
  ...
  const sendMessage = (mobile, msg) => {
    // Keep the same placeholder behaviour — console log for now
    console.log(`Sending message to ${mobile}: ${msg}`);
  };
  ...
  const addCustomer = async (e) => {
    ...
    try {
      ...
      sendMessage(
        mobile,
        messages.checkin?.replace("{name}", name)?.replace(
          "{restoName}",
          restoName
        )
      );
      ...
      setTimeout(() => {
        sendMessage(
          mobile,
          messages.checkout?.replace("{name}", name)?.replace(
            "{restoName}",
            restoName
          )
        );
  ...
  const filteredCustomers = customers.filter(([key, item]) => {
    ...
    const matchesFilter =
      filterValue === "" || item.gender.toLowerCase() === filterValue.toLowerCase();

    const matchesDate = selectedDate === "" || item.checkInDate === selectedDate;
  ...
  // -------------------- RESPONSIVE CUSTOMER TABLE (Option A)
  const ResponsiveCustomerTable = () => (
    <div className="w-full overflow-x-auto rounded-xl border border-gray-200">
      <table className="min-w-[700px] w-full border-collapse text-xs sm:text-sm">
        <thead className="bg-blue-100 text-gray-700">
          <tr>
            {["Name", "Mobile", "DOB", "Gender", "Actions"].map((head) => (
              <th
                key={head}
                className="p-3 text-left border-b whitespace-nowrap font-semibold"
              >
                {head}
              </th>
            ))}
        ...
        <tbody className="bg-white">
          {currentCustomers.length > 0 ? (
            currentCustomers.map(([key, item]) => (
              ...
              >
                <td className="p-3 whitespace-nowrap max-w-[200px] truncate">{item.name}</td>
                <td className="p-3 whitespace-nowrap">{item.mobile}</td>
                <td className="p-3 whitespace-nowrap">{item.dob}</td>
                <td className="p-3 whitespace-nowrap">{item.gender}</td>

                <td className="p-3 text-center whitespace-nowrap">
                  <div className="flex flex-wrap justify-center gap-2">
                    <button
                      onClick={() => editCustomerHandler(key, item)}
                      className="flex items-center gap-1 bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1.5 rounded-lg text-xs sm:text-sm shadow"
                    >
                      <Edit3 size={14} /> Update
                    </button>
                    ...
                    <button
                      onClick={() => deleteCustomer(key)}
                      className="flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-xs sm:text-sm shadow"
                    >
                      <Trash2 size={14} /> Delete
                    </button>
                  </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 font-poppins flex flex-col">
      ...
      <main className="flex-1 max-w-6xl mx-auto py-8 px-4 sm:px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              <div className="bg-white shadow p-6 rounded-2xl border border-gray-200">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-800 text-sm sm:text-base">
                  <p className="flex items-center gap-2">
                    <UserPlus className="w-4 h-4 text-indigo-600" />
                    <span className="font-semibold">Name:</span> {restoInfo.name}
                  </p>

                  <p className="flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-indigo-600" />
                    <span className="font-semibold">Email:</span> {restoInfo.email}
                  </p>

                  <p className="flex items-center gap-2">
                    <Phone className="w-4 h-4 text-indigo-600" />
                    <span className="font-semibold">Mobile:</span> {restoInfo.mobile}
                  </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {[
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className="text-base sm:text-lg font-semibold opacity-90">{card.title}</h3>
                        <p className="text-3xl sm:text-4xl font-bold mt-2">{card.count}</p>
                      </div>

                      <div className="bg-white/25 p-3 sm:p-4 rounded-full">{card.icon}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="w-full mb-6">
            <div className="bg-gradient-to-br from-blue-50 via-white to-blue-100 p-5 rounded-2xl shadow-md">
              <div className="bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden">
                {/* Header */}
                <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 text-center">
                  <h2 className="text-xl font-bold text-white">Welcome to {restoInfo.name}</h2>
                  <p className="text-blue-100 text-sm mt-1">Please complete your registration</p>
                </div>
        ...
        {/* Customer List */}
        {activeSection === "list" && (
          <div className="bg-white rounded-2xl shadow-lg p-5 sm:p-8 border border-gray-100 overflow-hidden">
            <h2 className="text-lg sm:text-xl font-semibold text-blue-700 mb-6 flex items-center gap-2">
              <Users /> Customer List
            </h2>
